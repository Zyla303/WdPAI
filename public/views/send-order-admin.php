...
<div class="container mt-24">
    <div class="row">
        <?php
        if(!isset($orders)){
            ?><meta http-equiv="Refresh" content="0.000001;url=sendOrderAdmin"><?php
        } else{
        ...
        <div class="col-sm-1 border-gold text-center">
        <b>Order sent</b><br>
            <form action="SendOrder?payment_date=<?= $order->getPaymentDate(); ?>&user_id=<?= $order->getUserId(); ?>;" method="POST">
                <button class=" button-creator mt-8"> Archive </button>
            </form>
        </div>
        <?php
            $step++;
