...
<div class="container mt-24">
    <div class="row">
        <?php
        if(!isset($orders)){
        ?><meta http-equiv="Refresh" content="0.000001;url=waitorderadmin"><?php
        } else{
        ...
        <div class="col-sm-5 border-gold">
            ...
            <b>Price:</b>Price: <?= round($priceA, 2); ?>zł<br>
        ...
        <b>Customize items: <span class="red"><?php if($order->getCustomized() == 1){ echo "yes"; }else{ echo "no"; } ?></span></b><br>

               <button class=" button-creator mt-8"> Download </button>
        </div>
        <div class="col-sm-1 border-gold text-center">
        <b>Recipt or<br>invoice</b><br>
               <button class=" button-creator mt-8"> Download </button>
        </div>
        <div class="col-sm-1 border-gold text-center">
        <b>Order ready</b><br>
            <form action="finishOrder?payment_date=<?= $order->getPaymentDate(); ?>&user_id=<?= $order->getUserId(); ?>;" method="POST">
                <button class=" button-creator mt-8"> Finish </button>
            </form>
        </div>
            <?php
            $step++;
